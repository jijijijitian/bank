...
int transfer::m_transfer(const int &fd, const char* pipe, MYSQL* mysql)
{
	...
	memcpy(&da.body.other_account_id, pipe + 20, 4);
	memcpy(&da.body.passwd, pipe + 24, 9);
	...
	sprintf_s(buff1, "insert into id%d values(%d, '%s', %lf, 0, %lf, '%s', NULL);", da.body.account_id, da.body.account_id, rep.body_rep.name, da.body.money, rep.body_rep.balance, days);
	...
	//修改对方的信息
	memset(buff1, 0, sizeof(buff1));
	sprintf_s(buff1, "select money, name from register where id = %d;", da.body.other_account_id);
	result = mysql_query(mysql, buff1);
	if (result != 0)
	{
		printf("select error: %s\n", mysql_error(mysql));
	...
	if (mysql_row = mysql_fetch_row(mysql_res))
	{
		money2 = atof(mysql_row[0]);
		money2 += da.body.money;
		strcpy_s(name2, mysql_row[1]);
	}
	else
	{
		cout << "对方用户不存在" << endl;
		rep.head_rep.action = -3;
		send(fd, (char*)&rep, sizeof(rep), 0);
		return -8;
	}

	memset(buff1, 0, sizeof(buff1));
	sprintf_s(buff1, "insert into id%d values(%d, '%s', 0, %lf, %lf, '%s', %d);", da.body.other_account_id, da.body.other_account_id, name2, da.body.money, money2, days, da.body.account_id);
	result = mysql_query(mysql, buff1);
	if (result != 0)
	{
		cout << "insert error: " << mysql_error(mysql) << endl;
	...
	memset(buff1, 0, sizeof(buff1));
	sprintf_s(buff1, "update register set money = %lf where id = %d;", money2, da.body.other_account_id);
	result = mysql_query(mysql, buff1);
	if (result != 0)
	{
		cout << "insert error: " << mysql_error(mysql) << endl;
