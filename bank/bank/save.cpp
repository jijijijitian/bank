...
int save::m_save(const int &fd, const char* pipe, MYSQL* mysql)
{
	cout << "从客户端" << fd << "收到存款请求！" << endl;
	...
	//获取用户名
	char buff1[1024];
	sprintf_s(buff1, "select money, name from register where id = %d;", da.body.account_id);
